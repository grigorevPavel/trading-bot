...
import { FlashLoanTaker__factory, TestToken__factory, TestTrader__factory, Token__factory, UniswapV2Factory, UniswapV2Factory__factory, UniswapV2Pair, UniswapV2Router02, UniswapV2Router02__factory, UniswapV2Trader__factory, WETH__factory } from '@/typechain'
...
export async function deployTraderFixture() {
    ...
    await tokenB.connect(signers.deployer).approve(router.address, ONE.mul(200_000))
    await tokenC.connect(signers.deployer).approve(router.address, ONE.mul(100_000))

    await tokenC.connect(signers.deployer).approve(newRouter.address, ONE.mul(100_000))
